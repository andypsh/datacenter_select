...
const tab = ref<"map" | "dashboard" | "compare" | "policy">("map");
  ...
  <div class="flex flex-col h-full bg-[#0b1220] text-[#e6ecff]">
    <header class="flex items-center justify-between px-6 py-3 border-b border-white/10 bg-[#0e1830]">
      <div>
        ...
        <p class="text-xs text-white/50">
          제14회 산업통상자원부 공공데이터 활용 아이디어 공모전 · 9개 지표 · 109개 시군구
        </p>
      </div>
      <nav class="flex gap-1 bg-white/5 rounded-lg p-1 text-sm">
        <button
          v-for="t in (['map','dashboard','compare','policy'] as const)"
          :key="t"
        ...
        >
          ...
          <span v-if="t === 'compare' && store.compareSet.size > 0"
            class="ml-1 text-xs px-1.5 py-0.5 bg-blue-500/30 rounded">
    ...
    <div v-if="store.error" class="m-6 p-4 bg-red-500/20 border border-red-500/40 rounded text-sm">
      ...
      <p class="mt-2 text-white/70">
        <code>03_분석/build_scores.py</code> 를 먼저 실행했는지 확인.
      </p>
    </div>
