...
V1 CSV 결과물을 V2 프론트엔드가 소비할 JSON으로 변환하는 Python 코드.

## 구성

- `coords.py` — 시군 이름 → 위경도 매핑 (109개, 수기 작성)
- `build_scores.py` — `../06_실거래가/가중치데이터_최종.csv` + `coords.py` → `../frontend/public/data/regions.json`
...
| 출처 | 컬럼 | 비고 |
|------|------|------|
| `06_실거래가/가중치데이터_최종.csv` | 시군, 평균기온, 지진위험도, 태풍위험도, 기업갯수, 거래금액 | 109개 시군 |
| `coords.py` | lat, lng | 수기 좌표 (시청·군청 위치 기준) |

## 다음 단계 (TODO)

- [ ] 좌표 데이터를 행정구역 GeoJSON으로 교체 (히트맵 → 폴리곤 채색)
- [ ] 실시간 데이터 (한국전력 OpenAPI, 기상청) 페치 스크립트
- [ ] 시계열 데이터 추가 (`02_계약종별전력사용량` 연도별 결과 활용)
